  ...
  return (
    <div>
       {belongTeam[0] && <div> 
        ...
        {belongTeam && belongTeam.map((item,idx)=>(<BelongTeam key={idx+item.team_name} item={item} idx={idx}></BelongTeam>))}

        {belongTeam[0] && <div> 
          <h3>가입 승인 대기 중인 팀</h3> 
        </div>}
        
        {waitingTeam && waitingTeam.map((item,idx)=>(<WaitingTeam key={idx+item.team_name} item={item} idx={idx}></WaitingTeam>))}
