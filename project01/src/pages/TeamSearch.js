...
const TeamSearch = ({ item, idx }) => {
	...
	const [nowGameCategory, setNowGameCategory] = useState('');
		...
		if (didMount.current && filterTeam.length !== 0) {
			axios
				...
				.then((e) => {
					...
					// 이름 필터링
					nameFilter = allTeam.filter((i) => i.team_game === nowGameCategory);
					...
					const genderFilter = positionFilter.filter(
						(x) =>
							x.gender_detail !== genderDetail[0] &&
							genderDetail[0] !== '제한없음'
					);
					// 나이 필터
	...
	return (
		<div>
			<Bubble
				setNowGameCategory={setNowGameCategory}
			...
			<div className='tsBoxFlex'>
				<div id='teamSearchBox'>
					{allTeam &&
						newTeam.map((item, idx) => <SearchTeam item={item} key={idx} />)}
				</div>
			</div>
		</div>
	);
